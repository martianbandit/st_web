...
with col1:
    st.title(f"{list0[0]}/{list0[1]}/{list0[2]}")
    st.title(f"{list0[3]}:{list0[4]} 登録")
...
with col2:
    st.subheader(f"消費量{consumption_rate}/hとして")
    if hour_later == 0:
        st.subheader(f"現在時刻 {later_.strftime('%H:%M')} の")
        st.subheader(f"予測値")
    else:
        st.subheader(f"今から{hour_later}時間後")
        st.subheader(f"{later_.strftime('%H:%M')}の予測値")
